...
class AWSCleaner(CloudCleaner):
    ...
    def cleanup_load_balancers(self):
        ...
        try:
            lbs = self.elb.describe_load_balancers()['LoadBalancerDescriptions']
            for lb in lbs:
                name = lb['LoadBalancerName']
                logger.info(f"Deleting Classic LB: {name}")
                self.elb.delete_load_balancer(LoadBalancerName=name)
        except Exception as e:
            logger.warning(f"Error deleting Classic LBs: {e}")
        ...
        try:
            ...
            logger.warning(f"Error deleting V2 LBs: {e}")
    ...
    def run_terraform_destroy(self):
        ...
        try:
            # Initialize to ensure backend is configured
            subprocess.run(
                ["terraform", "init", 
            ...
            # We continue even if TF fails, to try and clean up state buckets
    ...
    def cleanup_state_store(self):
        ...
        try:
            ...
            if bucket.creation_date:
                ...
                logger.info(f"Bucket {self.state_bucket} not found.")
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"DynamoDB table {self.lock_table} not found.")
        except Exception as e:
